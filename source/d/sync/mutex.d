...
public:
	void lock() shared {
		...
		size_t expected = 0;
		if (likely(word.casWeak!(MemoryOrder.Acquire)(expected, LockBit))) {
			return;
		}
	...
	bool tryLock() shared {
		size_t current = 0;
		if (likely(word.casWeak!(MemoryOrder.Acquire)(current, LockBit))) {
			return true;
		}
	...
	void unlock() shared {
		...
		if (likely(word.casWeak!(MemoryOrder.Release)(expected, 0))) {
			return;
		}
...
private:
	...
	void lockSlow() shared {
		...
		while (true) {
			...
			if (!(current & LockBit)) {
				...
				if (word.casWeak!(MemoryOrder.Acquire)(current, current | LockBit)) {
			...
			if (current == LockBit) {
				if (word.casWeak!(MemoryOrder.Release)(current, selfEnqueue(&wp) | LockBit)) {
					goto Handoff;
				}
			...
			if ((current & QueueLockBit) || !word
				    .casWeak!(MemoryOrder.Acquire)(current, current | QueueLockBit)) {
				sched_yield();
				continue;
			}
			...
			word.store!(MemoryOrder.Release)(enqueueLock(current, &wp) | LockBit);
	...
	void unlockSlowImpl(bool Fair)(WaitParams* wp) shared {
		...
		while (true) {
			...
			if (current == LockBit) {
				if (word.casWeak!(MemoryOrder.Release)(current, fastUnlock)) {
					return;
				}
			...
			if (word.casWeak!(MemoryOrder.Acquire)(current, current | QueueLockBit)) {
				break;
			}
		}
		 ...
		 */
		word.store!(MemoryOrder.Release)(current | Fair);
		...
		while (wakeList !is null) {
			...
			c.waitParams.handoff
			 .store!(MemoryOrder.Release)(Handoff.Barging + fair);
			c.waiter.wakeup();
	...
	static size_t selfEnqueue(WaitParams* wp) {
		...
		wp.handoff.store!(MemoryOrder.Release)(Handoff.None);
...
@"locking" unittest {
	...
	assert(mutex.word.load() == 0x00, "Invalid mutext state!");
	assert(count.load() == ts.length * 1024);
}
